...
export default function HistoricSalesCard({
  ...
  const fetchHistoricSales = async (forceFresh: boolean = false) => {
    ...
    try {
      ...
      } else if (data.dataSource === 'nsw-valuer-general') {
        toast.success(`Loaded ${salesCount} properties from NSW Valuer General data`);
      } else if (data.cached) {
        toast.success(`Loaded ${salesCount} properties from cache`);
      } else {
        toast.info(`Scraping ${salesCount} properties from Homely.com.au...`, { duration: 5000 });
      }
    } catch (err: any) {
  ...
  const uniqueProcessedSales = processedSales.filter(
    (sale, index, self) => index === self.findIndex(s => s.id === sale.id)
  );

  // Sort based on selected criteria
  const sortedSales = [...uniqueProcessedSales]
    .sort((a, b) => {
      switch (sortBy) {
        case 'distance':
...
`;
  ...
  useEffect(() => {
    if (!onSalesProcessed || uniqueProcessedSales.length === 0) return;
    ...
    const processedId = uniqueProcessedSales.map(s => `${s.id}:${s.similarity}`).join(',');
    ...
    const sortedForCallback = [...uniqueProcessedSales]
      .sort((a, b) => {
        const simDiff = b.similarity - a.similarity;
        if (simDiff !== 0) return simDiff;
        // When similarity is equal, prefer closer properties
  ...
  }, [uniqueProcessedSales, weightsLoaded, onSalesProcessed]);

  // Initialize Google Map when viewMode changes to 'map'
  useEffect(() => {
    if (viewMode !== 'map' || !mapRef.current || !googleLoaded) return;
  ...
  }, [viewMode, googleLoaded, sortedSales, propertyLatitude, propertyLongitude, propertyLocation, propertyBeds, propertyBaths]);
  ...
  return (
    <div className={`rounded-2xl shadow-sm border ${colors.border} p-3 sm:p-6`}>
      {/* Header */}
      <div className="flex flex-col gap-1 sm:gap-3 mb-3 sm:mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-3">
          <h2 className={`text-base sm:text-lg font-bold ${colors.title} flex items-center gap-1 sm:gap-2`}>
            <span className="text-lg sm:text-xl">🏠</span>
            <span className="hidden sm:inline">Historic Property Sales</span>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <select
              ...
              className="px-1.5 sm:px-3 py-1.5 sm:py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            >
              {sortOptions.map((opt) => (
            ...
            <button
              ...
              className={`flex items-center gap-1 px-1.5 sm:px-3 py-1.5 sm:py-2 rounded-lg font-semibold transition-colors text-xs sm:text-sm ${
                viewMode === 'map'
            ...
            >
              ...
              <span className="hidden sm:inline">{viewMode === 'map' ? 'Table' : 'Map'}</span>
            </button>
            {/* Copy button */}
            <button
        ...
        <div className="flex justify-end">
          <button
            ...
            disabled={loading}
            className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-1.5 sm:py-2 ${colors.button} text-white rounded-lg font-semibold transition-colors text-xs sm:text-sm disabled:opacity-50`}
            title="Refresh data from Homely (bypasses cache)"
          >
            {loading ? <Loader2 className="animate-spin" size={14} /> : <Home size={14} />}
      ...
      <p className="text-xs text-gray-500 dark:text-gray-300 mb-3">
        ...
        <span className="font-medium text-gray-700 dark:text-gray-100">{info?.suburb || 'this area'}</span>
        {info?.neighbouringSuburb && (
          <span className="text-gray-500 dark:text-gray-300">
            {' '}and <span className="font-medium text-blue-600 dark:text-blue-300">{info.neighbouringSuburb.suburb}</span>
          </span>
        )}
        .
        {info?.searchedAt && (
      ...
      {/* Content */}
      <div className={`${colors.bg} rounded-xl p-2 sm:p-5 border ${colors.border}`}>
        {loading ? (
          <div className="text-center py-8">
            <Loader2 className="animate-spin mx-auto mb-2" size={32} />
            <p className={colors.title}>Loading historic sales...</p>
          </div>
        ) : error ? (
          ...
          <div className="space-y-2 sm:space-y-3 max-h-96 overflow-y-auto">
            {sortedSales.map((sale, index) => (
              <div
                key={sale.id}
                className={`rounded-lg p-2 sm:p-4 border transition-colors ${
                  sale.isExactMatch
              ...
              >
                <div className="flex justify-between items-start gap-1 sm:gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Property details */}
                    <div className="flex items-center gap-1.5 sm:gap-3 mt-1 text-xs text-gray-500 dark:text-gray-400 flex-wrap">
                      {sale.beds && (
                        <span className="flex items-center gap-1">
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="font-bold text-emerald-600 dark:text-emerald-400 text-sm sm:text-lg">${sale.price?.toLocaleString()}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{sale.sold_date}</p>
                    {sale.distance != null && (
                      <span className="inline-block mt-1 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded-full text-xs font-semibold">
